...
public interface NewnoticeBoard_Mapper {
	//글입력처리
		...
		//전체 글개수
		final String SELECT_CNT_ALL="select count(1) from newnotice_board";
		
		@Select(SELECT_CNT_ALL)
		int getTotalCnt();
		
		//조회수 증가
		final String UPDATE_HITS="update newnotice_board set hits=#{hits}+1 where id_x=#{idx}";
		...
		final String SELECT_PAGE="SELECT * FROM (SELECT ID_X, SUBJECT, ID, CREATED_DATE,"
				...
				+ " FROM newnotice_BOARD ORDER BY CREATED_DATE DESC) WHERE page=#{page}" ;
		...
		ArrayList<NewnoticeBoard_Bean>getList(@Param("page") int page,
				@Param("rowsPerPage") int rowsPerPage);
		
	// 회원가입 글개수
	final String SELECT_SIGN_UP_CNT = "select count(1) from newnotice_board where category='signup'";
	...
	final String SELECT_SiGN_UP_PAGE = "SELECT * FROM (SELECT ID_X, SUBJECT, ID, CREATED_DATE,"
			+ " CONTENT, HITS, ceil(rownum/ #{rowsPerPage}) as page" + " FROM newnotice_BOARD WHERE CATEGORY='signup' "
	...
	ArrayList<NewnoticeBoard_Bean> getSignUpList(@Param("page") int page, @Param("rowsPerPage") int rowsPerPage);

	// 예적금 글개수
	final String SELECT_SAVINGS_CNT = "select count(1) from newnotice_board where category='savings'";
	...
	final String SELECT_SAVINGS_PAGE = "SELECT * FROM (SELECT ID_X, SUBJECT, ID, CREATED_DATE,"
			+ " CONTENT, HITS, ceil(rownum/ #{rowsPerPage}) as page" + " FROM newnotice_BOARD WHERE CATEGORY='savings' "
			+ " ORDER BY CREATED_DATE DESC) WHERE page=#{page}";
	...
	@Results(value = { @Result(property = "idx", column = "id_x"), @Result(property = "subject", column = "subject"),
			@Result(property = "id", column = "id"), @Result(property = "created_date", column = "created_date"),
			@Result(property = "content", column = "content"), @Result(property = "hits", column = "hits") })
	ArrayList<NewnoticeBoard_Bean> getSavingsList(@Param("page") int page, @Param("rowsPerPage") int rowsPerPage);

	// 기타 글개수
		final String SELECT_ETC_CNT = "select count(1) from newnotice_board where category='etc'";
		...
		final String SELECT_ETC_PAGE = "SELECT * FROM (SELECT ID_X, SUBJECT, ID, CREATED_DATE,"
				+ " CONTENT, HITS, ceil(rownum/ #{rowsPerPage}) as page" + " FROM newnotice_BOARD WHERE CATEGORY='etc' "
				+ " ORDER BY CREATED_DATE DESC) WHERE page=#{page}";
		...
		final String SELECT_BY_ID="select id_x,subject, id, created_date, content, hits, filename"
				+ " from newnotice_board where id_x=#{idx}";
		...
		// 수정
		final String UPDATE_BY_ID="update newnotice_board set subject = #{subject},category=#{category}, "
		...
		//글 삭제
		final String DELETE_BY_ID="delete from newnotice_board where id_x=#{idx}";

...
public interface DebateBoard_Mapper {
	...
	final String SELECT_PAGE =
			...
			+ "from (select * from tlb_debate_board order by ref desc, step asc))"
			+ "where page=#{page}";
	@Select(SELECT_PAGE)
	@Results(value= {
	...
	ArrayList<DebateBoard_Bean>getList(@Param("page") int page,@Param("rowsPerPage") int rowsPerPage);
	
	//전체글 수
	final String SELECT_CNT_ALL =
			"select count(1) from tlb_debate_board";
	@Select(SELECT_CNT_ALL)
	int getTotalCnt();
	
	//조회수 증가
	final String UPDATE_HITS="update tlb_debate_board set hits=#{hits}+1 where idx_num=#{idx_num}";
	@Update(UPDATE_HITS)
	...
	final String SELECT_BY_ID="select idx_num, subject, id, created_date, content, hits, filename"
			+ " from tlb_debate_board where idx_num=#{idx_num}";
	@Select(SELECT_BY_ID)
	//				output
	@Results(value= {
			@Result(property = "idx_num",column="idx_num"),
	...
	//						input
	DebateBoard_Bean getSpecificRow(@Param("idx_num") int idx_num);
	
	//글 수정
	final String UPDATE_BY_ID="update tlb_debate_board set subject = #{subject}, "
	...
	//글 삭제
	final String DELETE_BY_ID="delete from tlb_debate_board where idx_num=#{idx_num}";
	@Delete(DELETE_BY_ID)
	void deleteSpecificRow(@Param("idx_num") int idx_num);
	
	//검색한 글 수
	final String SELECT_CNT_BY_SUBJECT="select count(1) from tlb_debate_board where "
	...
	final String SELECT_ROWS_BY_SUBJECT=
		"SELECT * FROM (SELECT idx_num, SUBJECT, ID, CREATED_DATE,"
					...
					+ " FROM tlb_debate_board WHERE SUBJECT LIKE '%'||'${likeThis}'||'%' "
	...
	////////////////////답변형 게시판/////////////////////////////
	//idx_num값을 업데이트 합니다.
	final String UPDATE_RE_WRITE="update tlb_debate_board set ref=#{idx_num} where idx_num=#{idx_num}";
	@Select(UPDATE_RE_WRITE)
	void updateRewrite(@Param("idx_num") int idx_num);
	
	//가장 최근글을 가져옵니다.
	final String RECENT_ID="select max(idx_num) from tlb_debate_board";
	...
	final String UPDATE_GROUP_STEP="update tlb_debate_board set step=step+1 where ref=#{ref} and step>#{step}";
	...
	final String GET_IDNAME = 
			"select id from tlb_debate_board where idx_num=#{idx_num}";
	@Select(GET_IDNAME)
	@Results(value = {
	@Result(property = "id",column="id")
	})
	debateBoard_Bean getIdname(@Param("idx_num") int idx_num);
*/
}

...
public interface ReviewBoard_Mapper {
	//글입력처리
		final String INSERT=
				"insert into review_board(best_idx, id_x, id, subject, created_date, review_case, "
				+"content, filename, filesize, step, ref, depth, satisfaction, banks, re_productname) "
				+"values (best_seq_idx.nextval, invest_seq.nextval,#{id},#{subject},SYSDATE, #{review_case}, " 
				+"#{content},#{filename},#{filesize}, #{step}, #{ref}, #{depth}, #{satisfaction}, #{banks}, #{re_productname})";		
		@Insert(INSERT)

...
public interface ReviewBoard_Mapper {
	//글입력처리
		final String INSERT="insert into review_board(id_x, id, subject, created_date, category, content, filename, filesize)"
				+ " values(review_seq.nextval,#{id},#{subject},SYSDATE, #{category},#{content},#{filename},#{filesize})";
		...
		final String SELECT_PAGE="SELECT * FROM (SELECT ID_X, SUBJECT, ID, CREATED_DATE,"
				...
				+ " FROM review_BOARD ORDER BY CREATED_DATE DESC) WHERE page=#{page}" ;
	...
	// 회원가입 글개수
	final String SELECT_SIGN_UP_CNT = "select count(1) from review_board where category='review_deposit'";
	...
	final String SELECT_SiGN_UP_PAGE = "SELECT * FROM (SELECT ID_X, SUBJECT, ID, CREATED_DATE,"
			+ " CONTENT, HITS, ceil(rownum/ #{rowsPerPage}) as page" + " FROM review_BOARD WHERE CATEGORY='review_deposit' "
	...
	// 예적금 글개수
	final String SELECT_SAVINGS_CNT = "select count(1) from review_board where category='review_savings'";
	...
	final String SELECT_SAVINGS_PAGE = "SELECT * FROM (SELECT ID_X, SUBJECT, ID, CREATED_DATE,"
			+ " CONTENT, HITS, ceil(rownum/ #{rowsPerPage}) as page" + " FROM review_BOARD WHERE CATEGORY='review_savings' "
			+ " ORDER BY CREATED_DATE DESC) WHERE page=#{page}";
	...
	// 기타 글개수
		final String SELECT_ETC_CNT = "select count(1) from review_board where category='review_etc'";
		...
		final String SELECT_ETC_PAGE = "SELECT * FROM (SELECT ID_X, SUBJECT, ID, CREATED_DATE,"
				+ " CONTENT, HITS, ceil(rownum/ #{rowsPerPage}) as page" + " FROM review_BOARD WHERE CATEGORY='review_etc' "
				+ " ORDER BY CREATED_DATE DESC) WHERE page=#{page}";
		...
		// 수정
		final String UPDATE_BY_ID="update review_board set subject = #{subject},category=#{category}, "

...
public interface QNABoard_Mapper {
	//글입력처리
		final String INSERT="insert into qna_board(board_idx, id_x, category_num, id, subject, created_date, reply,category, content, filename, filesize)"
				+ " values(board_seq_idx.nextval, faq_seq.nextval, #{category_num}, #{id},#{subject},SYSDATE,#{reply}, #{category},#{content},#{filename},#{filesize})";

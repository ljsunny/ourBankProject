...
public interface NewsBoard_Mapper {
	//글입력처리
		...
		//전체 글개수
		final String SELECT_CNT_ALL="select count(1) from news_board";
		
		@Select(SELECT_CNT_ALL)
		int getTotalCnt();
		
		//조회수 증가
		final String UPDATE_HITS="update news_board set hits=#{hits}+1 where id_x=#{idx}";
		...
		final String SELECT_PAGE="SELECT * FROM (SELECT ID_X, SUBJECT, ID, CREATED_DATE,"
				...
				+ " FROM news_BOARD ORDER BY CREATED_DATE DESC) WHERE page=#{page}" ;
	...
	// 회원가입 글개수
	final String SELECT_SIGN_UP_CNT = "select count(1) from news_board where category='signup'";
	...
	final String SELECT_SiGN_UP_PAGE = "SELECT * FROM (SELECT ID_X, SUBJECT, ID, CREATED_DATE,"
			+ " CONTENT, HITS, ceil(rownum/ #{rowsPerPage}) as page" + " FROM news_BOARD WHERE CATEGORY='signup' "
	...
	// 예적금 글개수
	final String SELECT_SAVINGS_CNT = "select count(1) from news_board where category='savings'";
	...
	final String SELECT_SAVINGS_PAGE = "SELECT * FROM (SELECT ID_X, SUBJECT, ID, CREATED_DATE,"
			+ " CONTENT, HITS, ceil(rownum/ #{rowsPerPage}) as page" + " FROM news_BOARD WHERE CATEGORY='savings' "
			+ " ORDER BY CREATED_DATE DESC) WHERE page=#{page}";
	...
	// 기타 글개수
		final String SELECT_ETC_CNT = "select count(1) from news_board where category='etc'";
		...
		final String SELECT_ETC_PAGE = "SELECT * FROM (SELECT ID_X, SUBJECT, ID, CREATED_DATE,"
				+ " CONTENT, HITS, ceil(rownum/ #{rowsPerPage}) as page" + " FROM news_BOARD WHERE CATEGORY='etc' "
				+ " ORDER BY CREATED_DATE DESC) WHERE page=#{page}";
		...
		final String SELECT_BY_ID="select id_x,subject, id, created_date, content, hits, filename"
				+ " from news_board where id_x=#{idx}";
		...
		// 수정
		final String UPDATE_BY_ID="update news_board set subject = #{subject},category=#{category}, "
		...
		//글 삭제
		final String DELETE_BY_ID="delete from news_board where id_x=#{idx}";
		
		@Delete(DELETE_BY_ID)
		void deleteSpecificRow(@Param("idx") int idx);
		
		// 검색한 글 수
		final String SELECT_CNT_BY_SUBJECT="select count(1) from news_board where "
		...
		final String SELECT_ROWS_BY_SUBJECT=
			"SELECT * FROM (SELECT ID_X, SUBJECT, ID, CREATED_DATE,"
						+ " CONTENT, HITS, ceil(rownum/ #{rowsPerPage}) as page"
						+ " FROM news_BOARD WHERE SUBJECT LIKE '%'||'${likeThis}'||'%' "
						+ " ORDER BY CREATED_DATE DESC) WHERE page=#{page}";

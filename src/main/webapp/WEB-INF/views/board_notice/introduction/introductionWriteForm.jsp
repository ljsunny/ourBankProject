...
<!-- *********************** 내용 ****************************  -->
<c:url var="insertUrl" value="/introduction_write_form.do" />	
<sf:form modelAttribute="boardBean" commandName="boardBean" enctype="multipart/form-data" method="POST" action="${insertUrl}" >
	...
	<table width="400" border="1" cellspacing="0" cellpadding="5">
		<tr>
			<td><b>제목</b></td>
			<td><sf:input path="subject" size="50" maxlength="50"  value="[공지사항]"/><br /> 
			<sf:errors path="subject" cssClass="error" /></td>
		</tr>
		<tr>
			<td><b>카테고리</b></td>
			<td><sf:select path="category">
				<sf:option value="signup" label="은행별 공지"/>
				<sf:option value="savings" label="상품별 공지"/>
				<sf:option value="etc" label="기타공지"/>
			</sf:select></td>
		</tr>
		<tr>
			<td><b>내용</b></td>

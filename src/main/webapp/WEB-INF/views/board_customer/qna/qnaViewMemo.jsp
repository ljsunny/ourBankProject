...
<table  class="tlb_board">
		<tr>
			...
			<c:out value="${boardData.getSubject()}" /></td>
			<td style="background-color: #f2f2f2"><b>조회수</b></td>
			<td style="background-color: #f2f2f2">
			<c:out value="${boardData.getHits()}" /></td>
		</tr>
		<tr>
			<td style="background-color: #f2f2f2"><b>이름</b></td>
			<td style="background-color: #f2f2f2">
			<c:out value="${boardData.getId()}" /></td>
			<td style="background-color: #f2f2f2"><b>작성일</b></td>
			<td style="background-color: #f2f2f2">
			<c:out value="${boardData.getCreated_date()}"></c:out></td>
		</tr>
		<tr>
...
<div class="div_board_bnt">
	...
	<c:if test="${uid=='admin'}">
		<!-- 관리자만 작성할 수 있음 -->
		<input type="button" value="답글" onclick="javascript:boardreply()" class="bnt_view">
	</c:if>

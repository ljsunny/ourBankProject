...
<%@page session="false"%>
<%@page import="com.ourbank.app.bean.ReviewBoard_Bean"%>
...
function review_depositlist(){
	location.href='reviewdepositList.do?current_page=1';
}
function review_savingslist(){
	location.href='reviewSavingsList.do?current_page=1';
}
function review_etclist(){
	location.href='reviewEtcList.do?current_page=1';
}
//리스트 _ 제목링크 누를 시 
 ...
 <div id="site_div">
			<div class="comunity_top_menu" >
				<div>
					...
					<button type="button" onclick="javascript:review_depositlist()" class="bnt_comu">예금리뷰</button>
					<button type="button" onclick="javascript:review_savingslist()" class="bnt_comu">적금리뷰</button>
					<button type="button" onclick="javascript:review_etclist()" class="bnt_comu">기타리뷰</button>
				</div>
				<div style="float: right; width: 50%; vertical-align: center">
					<form name=searchf method=post action="reviewSearch.do" id="searchf">
						<input type="text" name="searchStr" size="35" maxlenght="50">
...
<table cellspacing="1" width="700" class="page">
	<tr>
		<td><c:forEach var="i" begin="1" end="${t_pages}">
				<a href="reviewList.do?current_page=${i}"> [ <c:if
						test="${i==c_page}">
						<b>

...
<%@page session="false"%>
<%@page import="com.ourbank.app.bean.MeetBoard_Bean"%>
...
<script>
function totallist(){
	location.href='meetList.do?current_page=1';
}
function signuplist(){
	location.href='meetSignUpList.do?current_page=1';
}
function savingslist(){
	location.href='meetSavingsList.do?current_page=1';
}
function etclist(){
	location.href='meetEtcList.do?current_page=1';
}
</script>
</head>
<!-- *********************** 게시판 글쓰기 폼 ****************************  -->	
	...
	<tr>
		<td>
				<div style="float: left; width: 40%; padding:10px;" >
					...
					<button type="button" onclick="javascript:signuplist()">지역별</button>
					<button type="button" onclick="javascript:savingslist()">연령별</button>
					<button type="button" onclick="javascript:etclist()">기타</button>
				</div>
				<div style="float: right; width: 50%; vertical-align: center">
					<form name=searchf method=post action="meetSearch.do">
	...
	<c:forEach var="board" items="${boardList}">
		<tr>
			...
			<td width="320">
				<p align="center">
					<a
						href="meetView.do?idx=${board.getIdx()}
...
<table cellspacing="1" width="700" border="1">
	<tr>
		<td><c:forEach var="i" begin="1" end="${t_pages}">
				<a href="meetList.do?current_page=${i}"> [ <c:if
						test="${i==c_page}">
						<b>
...
<table cellspacing="1" width="700">
	<tr>
		<td><input type="button" value="글쓰기"
			onclick="window.location='meet_show_write_form.do'"></td>
	</tr>
</table>
<%-- </c:if> --%>
</div>	
